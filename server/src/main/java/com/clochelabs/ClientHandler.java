...
public class ClientHandler extends Thread {
    ...
    private Packet sendKey(GetKeyPacket request) {
        ...
        try {
            if(Urne.getInstance().getKeys().get(request.getIdRef()) != null){
                return new GiveKeyPacket(Urne.getInstance().getKeys().get(request.getIdRef()));
            }
        } catch (Exception e) {
            System.out.println("User tried to get a key that doesn't correspond to a poll");
            return new ErrorPacket("Error while getting the key, there is no current poll for this id");
        }
        return new ErrorPacket("Error while getting the key");
    }
